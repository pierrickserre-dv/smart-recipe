...
class RecipeResponse(BaseModel):
    ...
    def check_ingredients(cls, v: List[str], info: ValidationInfo):
        ...
        allowed = [i.lower().strip() for i in info.context.get("allowed_ingredients", [])]
        staples = {"salt", "pepper", "oil", "water", "butter", "olive oil", "garlic", "flour", "sugar"}
    ...
    def check_instruction_hallucinations(cls, v: List[str], info: ValidationInfo):
        if info.context is None: return v
        allowed_pantry = [i.lower().strip() for i in info.context.get("allowed_ingredients", [])]
        staples = {"salt", "pepper", "oil", "water", "butter", "olive oil", "garlic", "sugar", "flour"}
        all_authorized = set(allowed_pantry).union(staples)
        
        forbidden_pool = {"cream", "milk", "cheese", "wine", "honey", "saffron", "parsley", "lemon", "lime"}
