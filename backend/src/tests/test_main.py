...
from src.generation.schemas import RecipeResponse, RecipeRequest
...
def test_auth_invalid_scheme():
    ...
    response = client.post("/generate", json={"ingredients": ["tomato"]}, headers=headers)
    assert response.status_code == 401
...
def test_request_malformed_json_body(override_auth):
    response = client.post("/generate", content="{'ingredients': ['salt']}", headers={"Content-Type": "application/json"})
    assert response.status_code == 422
...
def test_validator_handles_staple_casing_and_whitespace(ingredient):
    data = {
        "title": "Staple Test", "prep_time": "1m", "difficulty": "Easy",
...
def test_validator_complex_substring_protection():
    data = {
        "title": "Oil Test", "prep_time": "1m", "difficulty": "Easy",
...
def test_service_unhandled_error(mocker, override_auth):
    mocker.patch("main.recipe_service.generate_recipe", side_effect=RuntimeError("API Crash"))
    response = client.post("/generate", json={"ingredients": ["water"]})
    assert response.status_code == 500
...
def test_idempotency_simulation(mocker, override_auth):
    mock_recipe = {
        "title": "Repeatable", "prep_time": "1m", "difficulty": "Easy",
        "ingredients_used": ["salt"], "instructions": ["Done"]
    }
...
def test_instruction_scrubber_full_logic_check():
    ...
    with pytest.raises(ValueError, match="hallucinated"):
        RecipeResponse.model_validate(raw_ai_output, context={"allowed_ingredients": ["water"]})
...
def test_app_recovers_after_fail(mocker, override_auth):
    mocker.patch("main.recipe_service.generate_recipe", side_effect=[
        Exception("Transient Fail"), 
        {"title": "Success", "prep_time": "1m", "difficulty": "Easy", "ingredients_used": ["salt"], "instructions": ["Step"]}
    ])
