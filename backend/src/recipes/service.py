...
class RecipeAIService:
    ...
    def generate_recipe(self, data: RecipeRequest) -> RecipeResponse:
        prompt = (
            ...
            "and/or garlic."
            "For each ingredient used, provide a specific quantity and unit"
            "(e.g., 200g, 1 tbsp, 2 cloves)."
            "Ensure the recipe instructions refer to these exact quantities."
            "Provide the response in a structured JSON format matching the"
            "schema. For ingredients_used, provide a list of objects with"
            "name (the ingredient name) and quantity (the amount with its unit)."
